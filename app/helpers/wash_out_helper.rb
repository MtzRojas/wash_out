        ...
        xml.tag! "s:complexType", :name => param.basic_type do
          ...
          if elems.any?
            xml.tag! "s:sequence" do
              elems.each do |value|
                xml.tag! "s:element", wsdl_occurence(value, false, :name => value.name, :type => value.namespaced_type)
              end
            end
          end

          attrs.each do |value|
            xml.tag! "s:attribute", wsdl_occurence(value, false, :name => value.attr_name, :type => value.namespaced_type)
          end
        end
